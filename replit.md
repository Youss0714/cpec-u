...
- **Gestion des Réclamations**: Official grade dispute workflow with per-evaluation contestation. Admin configures dispute periods per semester (open/close dates, teacher response deadline). Students submit claims via a 4-step wizard: (1) select subject, (2) select individual evaluations to contest via checkboxes, (3) type + motif + attachment, (4) recap + confirm. Claims stored with `contestedEvaluations` (jsonb array of `{evaluationNumber, currentGrade}`) and `proposedEvaluations` (jsonb array of `{evaluationNumber, proposedGrade}`). Unique claim number (REC-YYYY-NNNN) generated on submission. 3-step circuit: teacher responds (accept with per-evaluation proposed grades / reject with justification / transmit to admin), then admin arbitrates (validate, override, close). Per-evaluation grade corrections applied in `grades` table; new average recalculated from all evaluations. Full immutable audit log (`reclamation_history`). Status flow: soumise → en_cours → en_arbitrage → acceptée/rejetée/clôturée. **Push notifications at every step** with deep-link URLs (`/reclamations?id=X`, `/teacher/reclamations?id=X`, `/admin/reclamations?id=X`) — clicking a notification opens the specific reclamation detail. Stage-specific tags prevent notification overwriting. Notification points: student confirmation on submission, teacher notification with deadline, teacher_opened → student notified, teacher decision (accept/reject/transmit) → student + admins, admin decision → student. Service worker handles click-through navigation with `client.navigate()`. Admin dashboard with KPIs (total, pending, acceptance rate, avg resolution time, top subjects, top teachers by reclamation count). Tables: `reclamation_periods`, `reclamations` (with `contestedEvaluations` + `proposedEvaluations` jsonb columns), `reclamation_history`. Nav: student always sees "Mes Réclamations"; teacher sees "Réclamations" with pending badge; admin (scolarité + directeur) see "Réclamations" with pending badge. Security: duplicate submission returns 409; only own grades can be contested; period enforcement server-side.
